...
export class ListaUsuariosComponent implements OnInit {
  ...
  esAdminEmpresa = signal<boolean>(false);
  ...
  /**
   *  Verificar si el usuario logueado es administrador de empresa
   */
  verificarPermisos(): void {
    ...
    if (currentUser?.is_superuser) {
      this.esAdminEmpresa.set(true);
      return;
    }

    // Obtener el rol del usuario desde el servicio de usuarios
    this.usuarioService.getUsuarioById(currentUser?.user_id!).subscribe({
      next: (user) => {
        this.esAdminEmpresa.set(user.es_admin_empresa || false);
        console.log('👑 Es admin empresa:', this.esAdminEmpresa());
      },
      error: (error) => {
        console.error('Error al verificar permisos:', error);
        this.esAdminEmpresa.set(false);
      },
    });
  }
